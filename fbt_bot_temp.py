...
class BotSession:
    ...
    def get_chat_id(self):
        ...
        if str(self.current_group) not in group_db.keys():
            return False
...
async def add_group(update,context):
    ...
    temp_data = {}
    temp_data[chat_id] = {}
    dbh.update_db(temp_data,"group_data")
    dbh.poll()
    bot_logger.log_action("bot handler",f"Overlord has added group with chat_id:{chat_id}")
    await context.bot.send_message(update.effective_chat.id,
        "This group has been succesfully added to 'group_data'! Enjoy!")
...
async def remove_group(update,context):
    ...
    chat_id = update.message.chat_id
    group_db = dbh.load_db("group_data")
    removed_group = group_db.pop(str(chat_id))
    temp_user_data = {}
    temp_user_data[str(chat_id)] = removed_group
    dbh.update_db(temp_user_data,"removed_data")
    dbh.poll()
    dbh.update_db(group_db,"group_data",True)
    dbh.poll()
    bot_logger.log_action("bot handler",f"Overlord has removed group with chat_id:{chat_id}")
    await context.bot.send_message(update.effective_chat.id,f"This group has been successfully removed!\nThis probably sucks)")
...
async def toggle_respond(update: Update, context: CallbackContext) -> None:
    ...
    bot_logger.log_action("bot handler", f"Overlord has {status} user respond")
    await context.bot.send_message(update.effective_chat.id,f"I'm {status}!")
...
async def schedule_now(update: Update, context: CallbackContext) -> None:
    ...
    if c_session.use_info:
        ...
        info = False
    w,d,l = schedule.get_current_time()
    await context.bot.send_message(update.effective_chat.id,schedule.form_message(1,with_link=1,info=info,w=w,d=d,l=l),disable_web_page_preview=True)
...
async def schedule_today(update: Update, context: CallbackContext) -> None:
    ...
    if c_session.use_info:
        ...
        info = False
    w,d,l = schedule.get_current_time()
    await context.bot.send_message(update.effective_chat.id,schedule.form_message(2,with_link=1,info=info,w=w,d=d),disable_web_page_preview=True)
...
async def schedule_this_week(update: Update, context: CallbackContext) -> None:
    ...
    if c_session.use_info:
        ...
        info = False
    w,d,l = schedule.get_current_time()
    await context.bot.send_message(update.effective_chat.id,schedule.form_message(3,with_link=0,info=info,w=w),disable_web_page_preview=True)
...
async def schedule_all(update: Update, context: CallbackContext) -> None:
    ...
    if c_session.use_info:
        ...
        info = False
    await context.bot.send_message(update.effective_chat.id,schedule.form_message(4,with_link=0,info=info),disable_web_page_preview=True)
